...
# Ejemplo para 17 data sets del cristal llamado n47-d17
# murpholinox@gmail.com
for i in `seq -w 01 17`
do
  # Obviamente el directorio tiene que cambiar
  ...
  mv a XDS.INP
  # Corre XDS con los valores predeterminado del archivo de entrada
  xds_par 
  mkdir no_opt
  cp -p *.cbf *.LP *.XDS *.HKL no_opt/  
  cp -p CORRECT.LP CORRECT.LP.old 
  # Empieza la optimización
  ...
  # En este caso, tenemos un grupo espacial 96, XDS llega hasta el 89. Lo ayudamos.
  ...
  f2mtz HKLOUT temp.mtz<F2MTZ.INP
   ...
   END
EOF
echo "Listo el data set número $i"
done 
